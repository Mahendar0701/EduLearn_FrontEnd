...
export default function CourseListItems() {
  ...
  // Check if courses is an object and not an array
  if (typeof courses === "object" && !Array.isArray(courses)) {
    // Render the course details directly
    const course = courses;
    return (
      <div className="max-w-sm m-3 bg-slate-50 border border-gray-200 rounded-lg shadow-md">
        <Link to={`/dashboard/courses/${course.id}`}>
          <img
            className="rounded-t-lg"
            src={course.image}
  ...
  // If courses is not an object, assume it's an array and handle it as before
  if (isLoading) {
    return <span>Loading courses...</span>;
  }
  if (isError) {
